...
import { z } from 'zod'
import { getHfToken } from './request-context'
...
export function registerMcpServer(server: McpServer): void {
    ...
    server.registerTool(
        ...
        async ({ timezone }) => {
            ...
            return {
                content: [
                    { type: 'text' as const, text: `현재 시간: ${text} (${timezone})` }
                ],
                structuredContent: {
                    content: [
                        { type: 'text' as const, text: `현재 시간: ${text} (${timezone})` }
    ...
    server.registerTool(
        ...
        async ({ city, language }) => {
            ...
            try {
                ...
                const text = `${name} (${country}) → 위도 ${latitude.toFixed(
                    4
                )}, 경도 ${longitude.toFixed(4)}`
                return makeText(text)
            } catch (e) {
                return makeText(
                    `오류: 좌표 정보를 가져오지 못했습니다 (${(e as Error).message})`
                )
            }
        }
    )
    ...
    server.registerTool(
        ...
        async ({ latitude, longitude }) => {
            ...
            try {
                ...
                const text = `현재 날씨 (${latitude.toFixed(2)}, ${longitude.toFixed(
                    2
                )}): ${condition}, 기온 ${temperature_2m}°C, 습도 ${relative_humidity_2m}%, 풍속 ${wind_speed_10m} m/s`
                return makeText(text)
            } catch (e) {
                return makeText(
                    `오류: 날씨 정보를 가져오지 못했습니다 (${(e as Error).message})`
                )
            }
        }
    )
    ...
    server.registerTool(
        ...
        async ({ prompt, num_inference_steps }) => {
            ...
            if (!token) {
                const errorText =
                    '오류: HF_TOKEN이 없습니다. x-hf-token 헤더 또는 HF_TOKEN 환경변수를 설정하세요.'
                return {
                    content: [{ type: 'text' as const, text: errorText }]
                }
            }
    ...
    server.registerPrompt(
        'code-review',
        {
            title: 'Code Review',
            description:
                '코드를 입력받아 베스트 프랙티스에 따라 코드 리뷰를 수행하는 프롬프트 템플릿입니다.',
            argsSchema: {
        ...
        async (uri: URL) => {
            const info = {
                ...
                tools: [
                    ...
                    { name: 'get_time', description: '현재 날짜와 시간을 반환합니다.' },
                    {
                        name: 'geocode_city',
                        description: '도시 이름을 입력하면 위도와 경도 좌표를 반환합니다.'
                    },
